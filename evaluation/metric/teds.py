...
class CustomConfig(Config):
    ...
    def normalized_distance(self, *sequences):
        ...
        return float(
            distance.levenshtein(*sequences)) / self.maximum(*sequences)

    def rename(self, node1, node2):
        """Compares attributes of trees."""
        if (node1.tag != node2.tag) or (node1.colspan != node2.colspan) or (
...
class TEDS:
    ...
    def load_html_tree(self, node, parent=None):
        ...
        if node.tag == 'td':
            ...
            new_node = TableTree(node.tag, int(node.attrib.get('colspan',
                                                               '1')),
                                 int(node.attrib.get('rowspan', '1')), cell,
                                 *deque())
        else:
    ...
    def batch_evaluate(self, pred_json, true_json):
        ...
        if self.n_jobs == 1:
            scores = [
                self.evaluate(pred_json.get(filename, ''),
                              true_json[filename]['html'])
